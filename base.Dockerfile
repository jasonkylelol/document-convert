...
RUN cp -a /etc/apt/sources.list /etc/apt/sources.list.bak \
    ...
    && sed -i "s@https://.*ports.ubuntu.com@http://mirrors.huaweicloud.com@g" /etc/apt/sources.list \
    && apt-get -y update \
...
# 用户可以根据需要，替换成其他的CANN版本，但是同时也请替换相应的驱动.
# CANN下载地址: https://www.hiascend.com/software/cann/community-history
ARG CANN_TOOLKIT_NAME=Ascend-cann-toolkit_8.0.RC1_linux-aarch64.run
# install ascend CANN
RUN wget --no-check-certificate https://ascend-repo.obs.cn-east-2.myhuaweicloud.com/CANN/CANN%208.0.RC1/${CANN_TOOLKIT_NAME}
RUN cd /root && chmod 777 $CANN_TOOLKIT_NAME && ./$CANN_TOOLKIT_NAME --install --quiet

ENV LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/Ascend/driver/lib64:/usr/local/Ascend/driver/lib64/driver/:/usr/local/Ascend/driver/lib64/common/:/usr/local/Ascend/driver/lib64/stub:/usr/local/Ascend/ascend-toolkit/latest/acllib/lib64:/usr/local/Ascend/ascend-toolkit/latest/atc/lib64
